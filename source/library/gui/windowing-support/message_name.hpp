...
// Data source: <url: https://wiki.winehq.org/List_Of_Windows_Messages> ISO date 2020.05.09.
...
namespace winapi {
    using namespace cppx::basic_string_assembly;        // operator<<
    $use_cppx( C_str );
    $use_std( string, unordered_map );

    inline auto message_name( const UINT id )
        -> string
    {
        static const unordered_map<int, C_str> the_names =
        {
            {0x0000, "WM_NULL"},
            {0x0001, "WM_CREATE"},
            {0x0002, "WM_DESTROY"},
            ...
            // {0x0019, "WM_CTLCOLOR"},           // Only for 16-bit Windows. Replaced with WM_CTLCOLORBTN et al.
        ...
        if( the_names.count( id ) ) {
            return the_names.at( id );
        } else {
            return "<window message with id "s << id << ">";
        }
    }
}  // namespace winapi
